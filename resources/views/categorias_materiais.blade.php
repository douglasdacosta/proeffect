...
@if(isset($tela) and $tela == 'pesquisa')
    ...
    <div class="right_col" role="main">
        ...
        <form id="filtro" action="categorias-materiais" method="get" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="">
            <div class="form-group row">
            ...
            <div class="x_panel">
              ...
              <div class="x_content">
                <table class="table table-striped text-center">
                  ...
                  <tbody>
                  @if(isset($categorias_materiais))
                        @foreach ($categorias_materiais as $categorias_material)
                            <tr>
                            <th scope="row"><a href={{ URL::route($rotaAlterar, array('id' => $categorias_material->id )) }}>{{$categorias_material->id}}</a></th>
                              <td>{{$categorias_material->nome}}</td>
                              </tr>
                        @endforeach
                    @endif
                  </tbody>
...
@section('content')
        @if($tela == 'alterar')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-2">
                <input type="text" id="id" name="id" class="form-control col-md-7 col-xs-12" readonly="true" value="@if (isset($categorias_materiais[0]->id)){{$categorias_materiais[0]->id}}@else{{''}}@endif">
                </div>
            </div>
        @else
            @section('content_header')
        ...
        @endif
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                <input type="text" class="form-control" id="nome"  name="nome" value="@if (isset($categorias_materiais[0]->nome)){{$categorias_materiais[0]->nome}}@else{{''}}@endif">
                </div>
            </div>
            <div class="form-group row">
                <label for="status" class="col-sm-2 col-form-label"></label>
                <select class="form-control col-md-1" id="status" name="status">
                    <option value="A" @if (isset($categorias_materiais[0]->status) && $categorias_materiais[0]->status == 'A'){{ ' selected '}}@else @endif>Ativo</option>
                    <option value="I" @if (isset($categorias_materiais[0]->status) && $categorias_materiais[0]->status =='I'){{ ' selected '}}@else @endif>Inativo</option>
                </select>
            </div>
            <div class="form-group row">
                <div class="col-sm-5">
