                  ...
                  @if(isset($status))
                        @foreach ($status as $stat)
                            ...
                            <th scope="row"><a href={{ URL::route($rotaAlterar, array('id' => $stat->id )) }}>{{$stat->id}}</a></th>                              
                              <td>{{$stat->nome}}</td>
                              <td>@if ( $stat->status == 1 ) <span class='label label-success' >Ativo</span> @else  <span class='label label-danger' >Inativo</span> @endif </td>
                            </tr>
                        @endforeach
                    @endif                           
                  </tbody>
...
@section('content')
        ...
        @endif
            ...
            <div class="form-group row">
                <label  class="col-sm-2 col-form-label"> </label>
                <div class="custom-control custom-switch">
                    <input type="checkbox" class="custom-control-input" id="status" name="status" @if (!isset($status[0]->status) || $status[0]->status == 1) checked @else{{''}}@endif>
